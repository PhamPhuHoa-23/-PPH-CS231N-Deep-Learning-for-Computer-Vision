...
def rnn_step_backward(dnext_h, cache):
    ...
    # -> dloss/dWx = dnext_h/dwx dloss/dnext_h = da/dWx dnext_h/da dloss/dnext_h
    # -> dloss/dWx = x.T tanh' dout
    # -> dloss/dWx = x.T @ (1 - tanh(a)^2) * dout
    # -> dloss/dWx = x.T @ (1 - next_h^2) * dout (D,N) * (N,H)(N,H) = (D,H)

    # dloss/dWh = h.T @ (1-next_h**2) * dout
    # dloss/dx = (1-next_h**2) * dout @ Wx.T
    # dloss/dprev_h = (1-next_h**2) * dout @ Wh.T
    # dloss/db = ((1-next_h**2) * dout).sum(axis=0)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        cur_dh = dh[:,t,:] + dprev_h
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(cur_dh, cache[t])
        dx[:,t,:] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def word_embedding_backward(dout, cache):
    ...
    #     mask = (x == idx)
    #     dW[idx] = np.sum(dout[mask], axis=0)
