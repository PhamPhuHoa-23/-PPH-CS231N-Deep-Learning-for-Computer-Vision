...
class CaptioningTransformer(nn.Module):
    """
    A Captioning Transformer produces captions from image features using a
    Transformer decoder.
    ...
    def forward(self, features, captions):
        ...
        # 1. Embed caption and positional encoding, project image_features into the same dimension
        captions_embed = self.embedding(captions)
        captions_with_positions = self.positional_encoding(captions_embed)

        decoder_inputs = self.visual_projection(features).unsqueeze(1)

        # Prepare_mask (torch.trill())
        tgt_mask = torch.tril(torch.ones((N,T,T))).bool().float()

        decoder_outputs = self.transformer(
            tgt=captions_with_positions,
            memory=decoder_inputs,
            tgt_mask=tgt_mask
        )
        
        print(f"Scores: {scores.shape}")
        scores = self.output(decoder_outputs)
