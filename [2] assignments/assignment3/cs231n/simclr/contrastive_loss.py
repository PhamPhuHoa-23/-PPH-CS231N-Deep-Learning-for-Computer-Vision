...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # l(k,k+N) = -log ( exp(s(k,k+N) / tau) ) / ( sigma_i^2N [k'!=k] exp(s(k,k')/tau) )
        # l(k+N,k) = -log ( exp(s(k,k+N) / tau) ) / ( sigma_i^2N [k'!=k+N] exp(s(k+N,k')/tau) )
        s_ks = torch.zeros(size=(2*N,))
        s_k_Ns = torch.zeros(size=(2*N,))

        for i in range(2*N):
            s_ks[i] = sim(z_k, out[i]) / tau
            s_k_Ns[i] = sim(z_k_N, out[i]) / tau


        s_k_k_N = s_ks[k+N]
        s_k_N_k = s_k_Ns[k]

        s_ks = s_ks[torch.arange(0,2*N) != k]
        s_k_Ns = s_k_Ns[torch.arange(0,2*N) != k+N]

        l1 = - s_k_k_N + torch.log(torch.sum(torch.exp(s_ks)))
        l2 = - s_k_N_k + torch.log(torch.sum(torch.exp(s_k_Ns)))
...
def compute_sim_matrix(out):
    ...
    dot_product = torch.mm(out, out.transpose(1, 0))
    norm_left = torch.linalg.norm(out, dim=1, keepdim=True)
    norm_right = torch.linalg.norm(out, dim=1, keepdim=True).transpose(1,0)
    sim_matrix = dot_product / (norm_left * norm_right)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = sim_positive_pairs(out_left, out_right) / tau
    ...
    numerator = - torch.concatenate([pos_pairs, pos_pairs], dim=0)
    ...
    loss = torch.mean(numerator + torch.log(denom))
