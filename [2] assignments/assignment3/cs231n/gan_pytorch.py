...
def discriminator_loss(logits_real, logits_fake):
    ...
    # l_d = - log(logits_real) / N - log(1-logits_fake) / N

    N = logits_real.size(0)
    label_reals = torch.ones(N).type(dtype)
    label_fakes = torch.zeros(N).type(dtype)

    loss_real = bce_loss(logits_real, label_reals)
    loss_fake = bce_loss(logits_fake, label_fakes)
...
def generator_loss(logits_fake):
    ...
    label_fools = torch.ones(logits_fake.size(0)).type(dtype)
    loss = bce_loss(logits_fake, label_fools)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = 0.5 * torch.mean(torch.pow(scores_real - label_reals, 2))
    loss_fake = 0.5 * torch.mean(torch.pow(scores_fake - label_fakes, 2))
...
def ls_generator_loss(scores_fake):
    ...
    loss = 0.5 * torch.mean(torch.pow(scores_fake - torch.ones(scores_fake.size(0)).type(dtype), 2))
